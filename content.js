const apiUrl = 'https://Tweet.pro/api';
...
function addTweetButtons() {
    if (window.location.hostname === 'x.com') {
        document.querySelectorAll('article').forEach(post => {
            if (post.querySelector('.Tweet-btn')) return;
...
function prepareName() {
    ...
    name.parentElement.appendChild(getNewBlock("Tweet-name-suggestions"));
}

function prepareTicker() {
    const ticker = document.getElementById("ticker");
    ticker.parentElement.parentElement.appendChild(getNewBlock("Tweet-ticker-suggestions"));
}

function prepareLogo() {
    let showMore = findAllByTextAndCssSelector('image or video', '[for=image]')[0];
    showMore.parentElement.appendChild(getNewBlock("Tweet-logo-suggestions"));
}
...
async function startLoadingImageSuggestions() {
    const container = document.getElementById("Tweet-logo-suggestions");
    ...
    try {
        ...
        document.getElementById("error-logo").textContent = "Cant get Tweet.pro image suggestions, try again later";
    } finally {
...
async function startLoadingTextSuggestions() {
    const nameContainer = document.getElementById("Tweet-name-suggestions");
    ...
    const tickerContainer = document.getElementById("Tweet-ticker-suggestions");
    ...
    try {
        ...
        document.getElementById("error-name").textContent = "Cant get Tweet.pro AI name suggestions, try again later";
        document.getElementById("error-ticker").textContent = "Cant get Tweet.pro AI ticker suggestions, try again later";
        tickerSpinner.remove();
        nameSpinner.remove()
    }
}
...
async function toggleTweetButtons() {
    if (window.location.hostname === 'Tweet.pro') {
...
const observer = new MutationObserver(addTweetButtons);
